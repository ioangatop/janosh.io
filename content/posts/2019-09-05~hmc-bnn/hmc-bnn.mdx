...
def sample_chain(*args, **kwargs):
    """This is bulk of the computation. Using @tf.function here to compile
    tfp.mcmc.sample_chain significantly improves performance, especially when
    enabling XLA (Accelerated Linear Algebra).
...
kernel = tfp.mcmc.NoUTurnSampler(bimodal.log_prob, step_size=1e-3)
...
density_plot = go.Surface(x=x, y=x, z=bimodal.prob(domain).numpy().reshape(len(x), -1))
samples_plot = go.Scatter3d(x=chain[:, 0], y=chain[:, 1], z=bimodal.prob(chain))
fig = go.Figure(data=[density_plot, samples_plot])
fig.update_layout(height=700, title=go.layout.Title(text=f"step size: {step_size}"))
```
